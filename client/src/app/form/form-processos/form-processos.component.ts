...
@Component({
  ...
  styleUrls: ['./form-processos.component.css']
})

export class FormProcessosComponent {
  public model1: any;
  model2: any;
  public model3: any;
  public model5: any;
  ...
  search5 = (text$: Observable<string>) => {
    ...
    return merge(debouncedText$, inputFocus$, clicksWithClosedPopup$).pipe(
      map(term => (term === '' ? permissao : permissao.filter(v => v.toLowerCase().indexOf(term.toLowerCase()) > -1)).slice(0, 10))
    );
    // tslint:disable-next-line:semicolon
  };
  ...
  search6 = (text$: Observable<string>) => {
    ...
    return merge(debouncedText$, inputFocus$, clicksWithClosedPopup$).pipe(
      map(term => (term === '' ? rotinas : rotinas.filter(v => v.toLowerCase().indexOf(term.toLowerCase()) > -1)).slice(0, 10))
    );
    // tslint:disable-next-line:semicolon
  };
